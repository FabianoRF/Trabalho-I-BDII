...
public class Main {
    ...
    private static void pegarTodosParticipantes(ArrayList<Players> playersNoCampeonato) {
        for(int i = 0; i < NUMERO_DE_INSCRITOS; i++){
            playersNoCampeonato.add(new Players("player" + (i + 1), 30000));
        }
    }

    private static void encherMesa(ArrayList<Players> playersNoCampeonato, ArrayList<Players> playersNaMesa) {
        playersNaMesa.clear();
        int i, j, c = 0;
        for (i = 0; i < 9; i++){
